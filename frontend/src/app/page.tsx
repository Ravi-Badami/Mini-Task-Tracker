...
function HomeContent() {
      ...
      intervalId = setInterval(async () => {
        try {
          const res = await fetch(
            `http://localhost:5000/auth/check-verification-status?email=${encodeURIComponent(registeredEmail)}`
          );
          if (res.ok) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Hide resend link when user starts typing
    if (showResendLink) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMessage = data.message || `${isLogin ? 'Login' : 'Registration'} failed`;
        setMessage(errorMessage);
        setMessageType('error');

        // Show resend link only if login failed due to unverified email
        if (
          isLogin &&
  ...
  return (
    <main className='flex min-h-screen flex-col items-center justify-center p-24'>
      <div className='w-full max-w-md'>
        ...
        {/* Resend verification link - only shown after login failure due to unverified email */}
        {isLogin && showResendLink && (
